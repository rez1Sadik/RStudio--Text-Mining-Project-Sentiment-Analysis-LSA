...
# Calculate sentiment scores across multiple lexicons
out1 <- data.frame(
  data_amazon,
...
knitr::kable(head(out[,1:5]), caption = "Sample of Categorized Sentiment Scores")
...
knitr::kable(sorted_df, digits = 3, caption = "Lexicon Accuracy Rankings")
...
# Plot results
ggplot(sorted_df, aes(fct_reorder(Lexicon, Accuracy), Accuracy, fill = Lexicon)) +
  scale_fill_brewer(palette = "Paired", type = "qual") + 
  geom_col() +
  geom_text(aes(label = sprintf("%.1f%%", Accuracy * 100)), vjust = 0.5, hjust = 1.1, size = 4, color = "black", fontface = "bold") +
